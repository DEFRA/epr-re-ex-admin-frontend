...
> compatibility fEx: `docker build --platform=linux/arm64 --no-cache --tag epr-re-ex-admin-frontend`
...
```bash
docker run -p 3000:3000 epr-re-ex-admin-frontend:development
```
...
```bash
docker run -p 3000:3000 epr-re-ex-admin-frontend
```
