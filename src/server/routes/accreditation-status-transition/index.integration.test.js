...
describe('accreditation-status-transition', () => {
  ...
  describe('approve grant fields', () => {
    ...
    test('confirm page renders the valid from and valid to date inputs and the accreditation number field', async () => {
      vi.mocked(getUserSession).mockResolvedValue(mockUserSession)
    ...
    test('missing accreditation number re-renders the page with an error, preserving both dates', async () => {
      vi.mocked(getUserSession).mockResolvedValue(mockUserSession)
      const receivedBodies = []
    ...
    test('missing every field lists all three errors in field order', async () => {
      ...
      expect(summary).toContain('Enter the date the accreditation is valid to')
      expect(summary).toContain('Enter an accreditation number')
      expect(
        $('.govuk-error-summary a')
          .toArray()
          .map((a) => a.attribs.href)
      ).toEqual(['#valid-from-day', '#valid-to-day', '#accreditation-number'])
    })
